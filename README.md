...
| **Diagnostic downstream of desynchroniser** | A bad field shifts the parse position before the check runs; the user sees an opaque generic error instead of the specific one |
...
Keep the two layers separated (this is itself an entry in the anti-pattern library — *Separation of Concerns*):

| Layer | Responsibility | Content |
|---|---|---|
| **Trigger Config** (AGENTS.md / .cursorrules) | When to call | "Invoke /post-fix-review after fixing bugs" |
| **Skill Content** (SKILL.md) | What to do | The 5 questions, anti-pattern library, output format |
...
**Why this is wrong:** users copy 15+ lines instead of 3; Skill updates never propagate to user configs; the trigger config leaks execution logic. The Skill exists precisely so users don't need to know the 5 questions.

> **Meta-lesson:** when building tools for others, separate *"how to invoke"* from *"what happens inside"*. The caller should know the interface, not the implementation.
