...
| **诊断器排在失同步点下游** | 非法字段先偏移了流位置，校验来不及执行，用户只看到泛化的报错而非具体原因 |
...
两层职责必须分离（这本身就是反模式库里的一条 —— *关注点分离*）：

| 层 | 职责 | 内容 |
|---|---|---|
| **触发配置**（AGENTS.md / .cursorrules） | 何时调用 | "修复 Bug 后调用 /post-fix-review" |
| **Skill 内容**（SKILL.md） | 做什么 | 5 个问题、反模式库、输出格式 |
...
修复任何 Bug 后，依次回答这 5 个问题：
1. 根因：哪里出了问题？
2. 反模式：属于哪类错误？
……
```

**为什么错：** 用户要抄 15+ 行而不是 3 行；Skill 内容更新后用户配置不会跟着更新；触发配置混入了执行逻辑。Skill 存在的意义恰恰是让用户不需要知道这 5 个问题。

> **元教训：** 给别人做工具时，把「怎么调用」和「内部做什么」分开。调用方应只了解接口，而非实现。
